...
aa_4p7T(:,i)=a25mspulse(:,6);

fm_cal_matrix(:,i)=fm;
R1W_cal_matrix(:,i)=R1W_cal_obs;
AREX_amp_matrix(i) = (fs4.*ksw4.*(tt.*42.6*2*pi).^2./((tt.*42.6*2*pi).^2+(R2S4+ksw4)*ksw4));
AREX_width_matrix(i)=2*sqrt((tt.*42.6*2*pi).^2.*(R2S4+ksw4)/ksw4+(ksw4+R2S4)^2);
